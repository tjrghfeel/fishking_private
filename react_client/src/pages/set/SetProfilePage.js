...
export default inject("MemberStore")(
  observer(
    class extends React.Component {
      constructor(props) {
        super(props);
        this.fileProfile = React.createRef(null);
        this.state = {};
      }
      /********** ********** ********** ********** **********/
      /** functions **/
      ...
      onClickProfile = () => {
        this.fileProfile.current?.click();
      };
      onChangeProfile = async () => {
        if (this.fileProfile.current?.files.length > 0) {
          const file = this.fileProfile.current?.files[0];

          const {
            MemberStore: { REST_PUT_profileImage },
          } = this.props;
          const resolve = await REST_PUT_profileImage(file);
