  ...
  observer(
    class extends React.Component {
      ...
      loadPageData = async () => {
        ...
        if (loggedIn) {
          const {
            MyMenuStore: { REST_GET_myMenuPage },
          } = this.props;
          const resolve = await REST_GET_myMenuPage();
          this.setState({ ...resolve, isLoaded: true });
        }
      };
        ...
        return (
          <>
            ...
            <div className="container nopadding">
              ...
              <a
                onClick={() => {
                  if (loggedIn) history.push(`/coupon/available`);
                  else history.push(`/member/login`);
                }}
              >
              ...
              <nav className="nav nav-pills nav-sel nav-my nav-col-4">
                {this.menus.map((data, index) => (
                  <a
                    ...
                    className="nav-link"
                    onClick={() => {
                      if (!data.requiredLogin || loggedIn) {
            ...
            <ConfirmModal
              ...
              textOk={"통화"}
              onClickOK={() =>
                ExternalStore.linking(
                  "tel:".concat(process.env.REACT_APP_CS_PHONE)
                )
              }
            >
              <p>
