(function (window) {
  ...
  var jsmpeg = (window.jsmpeg = function (url, opts) {
    ...
    /*
	console.log("jsmpg url : " + url);
	console.log("jsmpg this.canvas : " + this.canvas);
	*/
    if (url instanceof WebSocket) {
      this.client = url;
      /*
		console.log("jsmpg this.client : " + this.client);
		*/
  ...
  jsmpeg.prototype.decodeSocketHeader = function (data) {
    // Custom header sent to all newly connected clients when streamingthis.sequenceStarted CCCC:
    // over websockets:
    // struct { char magic[4] = "jsmp"; unsigned short width, height; };
    console.log("ZZZZZZZZZZZZZZZZZZZ" + data);
    /*
	console.log(
	...
	*/
    if (
      ...
      /*
		console.log("YYYYYYYYYYYYYYYYYY" + data[4] + " / " + data[5]);
		console.log("YYYYYYYYYYYYYYYYYY" + data[6] + " / " + data[7]);
		*/
      /*
  ...
  jsmpeg.prototype.receiveSocketMessage = function (event) {
	...
	*/
    ...
    /*
	if(
		messageData[0] == SOCKET_MAGIC_BYTES.charCodeAt(0) &&
		messageData[1] == SOCKET_MAGIC_BYTES.charCodeAt(1) &&
		messageData[2] == SOCKET_MAGIC_BYTES.charCodeAt(2) &&
		messageData[3] == SOCKET_MAGIC_BYTES.charCodeAt(3)
	) {
		if(messageData[4] == 0) return;
	}
	else {
*/
    if (!this.sequenceStarted) {
      console.log("this.sequenceStarted CCCC: " + this.sequenceStarted);
    ...
    while (true) {
      ...
      /*
			console.log("startCode : " + startCode + " / BitReader.NOT_FOUND = " + BitReader.NOT_FOUND + " / " + (startCode == BitReader.NOT_FOUND || (next.index >> 3) > next.writePos));
	*/
    ...
    if (chunkEnd > next.chunkBegin) {
      ...
      current.bytes.set(
        next.bytes.subarray(next.chunkBegin, next.lastWriteBeforeWrap)
      );
...
*/
  ...
  jsmpeg.prototype.recordFrameFromCurrentBuffer = function () {
    ...
    this.recordBuffers.push(
      new Uint8Array(this.buffer.bytes.subarray(0, this.buffer.writePos))
    );
  };
  ...
  jsmpeg.prototype.load = function (url) {
    ...
    console.log("jsmpeg.prototype.load : " + url);
    request.send();
  };
  ...
  jsmpeg.prototype.IDCT = function () {
    ...
    for (i = 0; i < 8; ++i) {
      ...
      x1 = m0 - b1;
      x2 =
        (((blockData[2 * 8 + i] - blockData[6 * 8 + i]) * 362 + 128) >> 8) - b3;
      x3 = m0 + b1;
      y3 = x1 + x2;
      y4 = x3 + b3;
      y5 = x1 - x2;
